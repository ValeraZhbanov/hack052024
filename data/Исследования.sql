...
BEGIN
	...
	PERFORM event.СоздатьУведомление (
		(SELECT 'Утверждено новое расписание за ' || replace(replace(to_char(ДатаНачала, 'tmmonth yyyy'), 'я ', 'ь '), 'а ', ' ')
		 FROM sch.ПроектыРасписания
		 WHERE Код = _КодПроектаРасписания), 
		'https://dnd-dusa.ru/telegram/web/schedule', 
		(SELECT array_agg(DISTINCT users.id) 
		 FROM sch.ЗаписиВерсииРасписания
...
BEGIN
	...
	IF _Комментарий IS NOT NULL THEN 

		PERFORM event.СоздатьУведомление(
			(SELECT 
				Доктора.ФИО || ' уведомил о форс-мажорной ситуации:' ||
				E'\n' || '<strong>Дата:</strong> ' || to_char(ЗаписиВерсииРасписания.Дата, 'dd.MM.yyyy') || 
					E'\n' || '<strong>Версия:</strong> №' || ВерсииПроектовРасписания.Код::varchar || ' от ' || to_char(ВерсииПроектовРасписания.ДатаСоздания, 'dd.MM.yyyy') ||
					E'\n' || '<strong>Сообщение</strong>: ' || E'\n' || E'\n' || ЗаписиВерсииРасписания.Комментарий
			 FROM sch.ЗаписиВерсииРасписания
			 INNER JOIN sch.ВерсииПроектовРасписания ON ВерсииПроектовРасписания.Код = ЗаписиВерсииРасписания.КодВерсииПроектаРасписания
		...
		INSERT INTO public.system_notifications(title, text, ids, "for", active)
		SELECT 
			Доктора.ФИО || ' уведомил о форс-мажорной ситуации:', 
			'<strong>Дата:</strong> ' || to_char(ЗаписиВерсииРасписания.Дата, 'dd.MM.yyyy') || 
				'<br><strong>Версия:</strong> №' || ВерсииПроектовРасписания.Код::varchar || ' от ' || to_char(ВерсииПроектовРасписания.ДатаСоздания, 'dd.MM.yyyy') ||
				'<br><strong>Сообщение: </strong>' || ЗаписиВерсииРасписания.Комментарий,
			to_jsonb(2),
			3,
			true
		FROM sch.ЗаписиВерсииРасписания
...
SELECT 
	...
	ROW_NUMBER() OVER (PARTITION BY ПроектыРасписания.Код, ВерсииПроектовРасписания.Код ORDER BY НеделяПроекта::date) НомерНеделиПроекта,

	json_agg(json_build_object(
		'ТипИсследований', row_to_json(ТипыИсследований),
		'КоличествоИсследованийПрогнозируемое', COALESCE(СтатистикаОбращений.Значение, 0)::int,

		'КоличествоДокторвРеально', COALESCE(ЗаписиВерсииРасписания.КоличествоДокторов, 0)::int,
		'КоличествоЧасовРеально', COALESCE(EXTRACT (EPOCH FROM ЗаписиВерсииРасписания.КоличествоЧасов) / 60 / 60, 0)::int,
		'КоличествоИсследованийРеально', COALESCE(ЗаписиВерсииРасписания.КоличествоИсследований, 0)::int,
...
FROM sch.ПроектыРасписания
INNER JOIN sch.ВерсииПроектовРасписания ON ВерсииПроектовРасписания.КодПроектаРасписания = ПроектыРасписания.Код
...
CREATE OR REPLACE VIEW sch.СтатистикаДляПроекта1
AS
SELECT 
	ПроектыРасписания.Код КодПроекта,
	ВерсииПроектовРасписания.Код КодВерсииПроектаРасписания,
	ДеньПроекта::date Дата,
...
LEFT JOIN (
	SELECT
		ЗаписиВерсииРасписания.КодВерсииПроектаРасписания,
		ЗаписиВерсииРасписания.Дата Дата,
	    Исследования.КодТипаИсследования,
		COUNT(DISTINCT ЗаписиВерсииРасписания.КодДоктора) КоличествоДокторов,
		SUM(ТипыРабочихСмен.Продолжительность) КоличествоЧасов,
		SUM(Исследования.Количество) КоличествоИсследований
	...
	GROUP BY ЗаписиВерсииРасписания.КодВерсииПроектаРасписания,
	         ...
	         Исследования.КодТипаИсследования
	                    ...
	                    AND ЗаписиВерсииРасписания.КодТипаИсследования = ТипыИсследований.Код
	                    AND ЗаписиВерсииРасписания.Дата = ДеньПроекта
GROUP BY ПроектыРасписания.Код,
		 ВерсииПроектовРасписания.Код,
		 ДеньПроекта
ORDER BY ДеньПроекта;
...
SELECT
	ВерсииПроектовРасписания.Код КодВерсииПроектаРасписания,
	ВерсииПроектовРасписания.Утверждено,
	СменыНаМесяц.ДатаНачалаМесяца::date Диапазон,
...
INNER JOIN hr.НормыДокторовПодробно ON НормыДокторовПодробно.КодДоктора = ДоктораПодробно.Код

ORDER BY СменыНаМесяц.ДатаНачалаМесяца
