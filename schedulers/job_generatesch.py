...
def processing():
    ...
    try:
        ...
        ЧислоДнейПроекта = (Проект['ДатаОкончания'] - ДатаНачала).days


        ДоктраПоДняПоТипамИсследований = DbStore.execute_select_query_all("""
        SELECT
	        ДоктораПоТипамИсПоДням.ДатаПроекта::date,
	        json_agg(json_build_object('КодТипаИсследования', ДоктораПоТипамИсПоДням.КодТипаИсследования,'Доктора', ДоктораПоТипамИсПоДням.Доктора)) ДоктораПоТипамИсследований
        FROM (
	        SELECT 
		        ДатаПроекта, 
		        ТипыИсследований.Код КодТипаИсследования,
		        CASE WHEN MAX(Доктора.Код) IS NOT NULL THEN json_agg(json_build_object('Доктор', row_to_json(Доктора), 'ТипРабочейСмены', row_to_json(ТипыРабочихСмен)) ORDER BY МодальностиДокторов.Основная DESC) ELSE '[]'::json END Доктора
	        FROM sch.ПроектыРасписания
	        CROSS JOIN med.ТипыИсследований
	        CROSS JOIN generate_series(ПроектыРасписания.ДатаНачала + '1 year'::interval, ПроектыРасписания.ДатаОкончания + '1 year'::interval, '1 day'::interval) ДатаПроекта
	        LEFT JOIN hr.РабочиеСменыДокторов ON РабочиеСменыДокторов.Дата = ДатаПроекта
	        LEFT JOIN hr.ТипыРабочихСмен ON ТипыРабочихСмен.Код = РабочиеСменыДокторов.КодТипаРабочейСмены
	        LEFT JOIN hr.Доктора ON Доктора.Код = РабочиеСменыДокторов.КодДоктора
	        ...
	        LEFT JOIN med.Модальности ON Модальности.Код = МодальностиДокторов.КодМодальности
							         AND ТипыИсследований.КодМодальности = Модальности.Код
	        WHERE ПроектыРасписания.Код = %s
	        GROUP BY ДатаПроекта, ТипыИсследований.Код
        ) ДоктораПоТипамИсПоДням
        GROUP BY ДоктораПоТипамИсПоДням.ДатаПроекта		
        """, [КодПроекта])
